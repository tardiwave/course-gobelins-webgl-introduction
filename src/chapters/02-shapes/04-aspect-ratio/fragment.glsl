...
// Multiplying two bands keeps only the pixels lit on both axes.
...
void main() {
  ...
  // LEFT: 0.12 of the width is not 0.12 of the height, so this is a rectangle.
  float wrong = square(vUv, vec2(0.28, 0.5), 0.12);

  // RIGHT: scale x by the aspect ratio around the centre, so units match on both axes.
